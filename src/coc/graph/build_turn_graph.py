...
from coc.content.compiled import CompiledRuleset, load_compiled_ruleset
from coc.content.packages import PackageKind
from coc.content.registry import ContentRegistry
from coc.content.retrieval import search_registry_text_indexed
from coc.content.visibility import AccessMode
from coc.context_budget import (
    build_advisor_context,
    build_context_budget_snapshot,
    compact_context_budget_trace,
)
from coc.graph.state import GraphState
from coc.langchain.advisors import AdvisorContractMode, AdvisorRole, invoke_advisor
from coc.langchain.prompts import (
...
from coc.langchain.tools import build_langchain_tools
from coc.memory.projection import project_recent_summary
from coc.memory.store import SqliteStore
from coc.rules.compiled_resolver import RULES_PLUGIN_RUNTIME_VERSION, run_ruleset_resolver
from coc.rules.plugin_runtime import RulesDslPlugin, load_rules_plugin
from coc.scenario.director import (
    ScenarioPatchValidation,
    validate_scenario_director_decision,
)
from coc.scenario.runtime import load_or_initialize_world_state, sync_scene_details
from coc.security.redaction import redact_secrets
from coc.tools.content import load_content_span, search_content
from coc.tools.patches import WorldPatch, apply_world_patches
...
def retrieve_content_spans(state: GraphState) -> GraphState:
    ...
    if sqlite_path:
        ...
        from coc.content.retrieval import search_registry_text_with_diagnostics
...
def retrieve_context_parallel(state: GraphState) -> GraphState:
    ...
    with ThreadPoolExecutor(max_workers=2, thread_name_prefix="coc-context") as executor:
    ...
    *,
    ...
    def run_micro_gates(state: GraphState) -> GraphState:
        ...
        with ThreadPoolExecutor(
            ...
            thread_name_prefix="coc-micro-gate",
        ) as executor:
            futures = {
                executor.submit(
                    _run_micro_gate,
...
def build_parallel_review_and_memory_node(
    ...
    def review_and_curate_parallel(state: GraphState) -> GraphState:
        branch_state: GraphState = {**state, "trace_events": []}
        with ThreadPoolExecutor(max_workers=2, thread_name_prefix="coc-review") as executor:
